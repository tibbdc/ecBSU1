...
from cobra.io.dict import model_to_dict, gene_from_dict
...
def get_fluxes_detail_in_model(model,model_pfba_solution,fluxes_outfile,json_model_file):
    ...
    * reaction_kcat_mw_file: reaction kcat/mw file.
    ...
    for index, row in model_pfba_solution.iterrows():
        ...
        model_pfba_solution_detail.loc[index, 'equ'] = reaction_detail.gene_reaction_rule
...
def calculate_f_special(gene_abundance_file,modelgene2uniprot_file,paxdbgene2uniprot_file,gene_abundance_colname):
    ...
    * gene_mw_file: The molecular weight of the protein expressed by each gene.
    ...
    model_gene = pd.read_csv(modelgene2uniprot_file, sep='\t', index_col='Entry')
    paxdb_gene = pd.read_csv(paxdbgene2uniprot_file, sep='\t', index_col='Entry')
    paxdb_gene=paxdb_gene[paxdb_gene['Organism']=='Bacillus subtilis (strain 168)']
    model_gene=model_gene[model_gene['Organism']=='Bacillus subtilis (strain 168)']
    enzy_abundance = 0
    pro_abundance = 0
    for gene_i in gene_abundance.index:
        #print(gene_i)
...
def draw_cdf_fig(data_cdf_data,output_file,x_name,y_name,y_index,nticks):
    ...
    layout = go.Layout(plot_bgcolor='lightgrey',xaxis=dict(title=dict(text=x_name,font=dict(size=20, family='Times New Roman')),
            ...
            showlegend=False,height=450,width=750,margin=go.layout.Margin(l=0, r=0, b=0, t=0))
...
def excel2xml(name_excel, xml_model,json_model):
    excel = pd.ExcelFile(name_excel)
    model = Model()
    # Process the metabolites information
    # fillna('') is order to avoid nan value,otherwise,an error will occur when getting the model 
    try:
        mdictl = excel.parse('metabolites', index_col=None).fillna('').T.to_dict().values()
        for m in mdictl:
            annstr=m['annotation']
            values = re.search(r"OrderedDict\((.*)\)", annstr).group(1)
            anndict = collections.OrderedDict(ast.literal_eval(values))
            m['annotation']=anndict
        model.metabolites.extend([metabolite_from_dict(metabolite) for metabolite in mdictl])
    except BaseException:
        pass
    #model.add_metabolites([metabolite_from_dict(metabolite) for metabolite in mdictl])
    # Process the genes information
    # KEGG database has no genes information
    #try:
    ...
    #except BaseException:
        #pass
    # Process the reactions information
    rdictl = excel.parse('reactions', index_col=None).fillna('').T.to_dict().values()
    for r in rdictl:
        ...
        rea = reaction_from_dict(r, model)  
    ...
    save_json_model(model, json_model)
...
#     fig = go.Figure(data=[go.Surface(y=z_data.index,x=z_data.index,z=z_data.values)],layout=layout)
#     fig.update_layout(scene = dict(xaxis = dict(backgroundcolor = "lightgrey",title=dict(text="Glucose uptake rates<br>(mmol/gDW/h",
#                       font=dict(size=15, family='Times New Roman'),)),yaxis = dict(backgroundcolor = "lightgrey",
#                         title=dict(text="O2 uptake rates<br>mmol/gDW/h",font=dict(size=15, family='Times New Roman'))),
#                         zaxis = dict(backgroundcolor = "grey", gridcolor = "white", title=dict(text="Growth rates (1/h)",
...
def draw_3d_rbas(z_data,o2_bound,substrate_bound,number,out_fig_file):
    ...
    fig.update_layout(scene = dict(
        xaxis = dict(range=[0,o2_bound+0.2],backgroundcolor = "lightgrey",title=dict(text="<b>O2 uptake rates<br>(mmol/gDW/h)</b>",font=dict(size=15, family='Times New Roman'))),
        yaxis = dict(range=[0,substrate_bound+0.2], backgroundcolor = "lightgrey",title=dict(text="<b>Glucose uptake rates<br>(mmol/gDW/h)</b>",font=dict(size=15, family='Times New Roman'))),
        zaxis = dict(range=[0,1.4], backgroundcolor = "grey", gridcolor = "white", title=dict(text="<b>Growth rates<br>(1/h)</b>",font=dict(size=15, family='Times New Roman')))))
...
def get_robustness_data_GEM(model_file,model_type,obj,o2_bound,substrate_bound,number,outputfile):
    if model_type=='GEM':
    ...
    for i in exlistmo2:
        ...
        for j in exlistm:
            ...
            cg_model.reactions.get_by_id('EX_glc__D_e').bounds=(condj,0)
            #cg_model.reactions.get_by_id('EX_glc__D_e_reverse').bounds=(0,0)
...
def get_robustness_data_ecGEM(model_file,model_type,obj,o2_bound,substrate_bound,number,outputfile):
    if model_type=='GEM':
        cg_model = cobra.io.json.load_json_model(model_file)
    else:
        cg_model=get_enzyme_constraint_model(model_file)
